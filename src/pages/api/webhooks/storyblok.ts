...
export async function POST({ request, locals }: APIContext) {
  ...
  try {
    ...
    if (!purgeResponse.ok) {
      ...
      console.error(
        `Cloudflare API error: ${purgeResponse.status} ${errorText}`,
      );
      return new Response(`Error purging cache: ${errorText}`, {
    ...
    if (purgeResult.success) {
      ...
      return new Response(
        `Cache purge failed: ${JSON.stringify(purgeResult.errors)}`,
        { status: 500 },
      );
    }
  } catch (error) {
...
export async function GET() {
  return new Response(
    "Revalidation endpoint is active. Use POST to trigger revalidation. 111",
    { status: 200 },
  );
}
